...
from .stub import stub, cls_dec, MODELS_DIR, volume, HF_DATASETS_CACHE, REPO_HOME
...
class VideoLlava:
    ...
    @enter()
    def load_model(self):
        self.model, self.tokenizer = load_pretrained_from_cache()
        self.processor = prepare_processor(self.model)
...
def main():
    ...
    output = VideoLlava().inference.remote(video_path, prompt)
    print(output)
